...
/**
 * http://www.glassdoor.com/Interview/N-pots-each-with-some-number-of-gold-coins
 * -are-arranged-in-a-line-You-are-playing-a-game-against-another-player-You-tak
 * -QTN_350584.htm
 * 
 * @author tusroy
 * 
	...
	public Pair[][] findMoves(int pots[]) {
		...
		for (int l = 2; l <= pots.length; l++) {
			for (int i = 0; i <= pots.length - l; i++) {
				int j = i + l - 1;
				if (pots[i] + moves[i + 1][j].second > moves[i][j - 1].second
						+ pots[j]) {
