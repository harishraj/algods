...
/**
 * http://www.geeksforgeeks.org/full-and-complete-binary-tree-from-given-
 * preorder-and-postorder-traversals/ Test cases Empty tree Tree with big on
 * left side Tree with big on right side
 */
public class ConstructFullTreeFromPreOrderPostOrder {
	...
	private Node constructTree(int preorder[], int postorder[], int low,
			int high, int index) {
		...
		node.right = constructTree(preorder, postorder, i + 1, high - 1, index
				+ i - low + 2);
		return node;
	}
