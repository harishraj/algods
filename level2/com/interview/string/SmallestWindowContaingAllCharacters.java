...
public class SmallestWindowContaingAllCharacters {
	...
	private void updateMapDecreaseCount(Map<Character, Integer> map,
			Character ch) {
	...
	public int smallestWindow(char[] str, char[] subString) {
		...
		i = trimWindow(startIndex, str, endIndex, mainStringVisited,
				subStringVisited);
		...
		for (i = endIndex + 1; i < str.length; i++) {
			...
			if (str[startIndex] == str[i]) {
				startIndex = trimWindow(startIndex, str, i, mainStringVisited,
						subStringVisited);
	...
	private int trimWindow(int startIndex, char[] str, int endIndex,
			Map<Character, Integer> mainStringVisited,
			Map<Character, Integer> subStringVisited) {
		int i = 0;
	...
	public static void main(String args[]) {
		...
		int minWindow = swcac.smallestWindow(str.toCharArray(),
