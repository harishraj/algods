...
/**
 * http://www.geeksforgeeks.org/ford-fulkerson-algorithm-for-maximum-flow-
 * problem/
 */
public class FordFulkerson<T> {
	...
	public int maxFlow(Graph<T> graph, Vertex<T> s, Vertex<T> t) {
		...
		int[][] matrix = new int[graph.getAllVertex().size()][graph
				.getAllVertex().size()];
		for (Edge<T> edge : graph.getAllEdges()) {
			matrix[(int) edge.getVertex1().getId()][(int) edge.getVertex2()
	...
	private boolean BFS(int[][] matrix, Map<Integer, Integer> parent, int s,
			int t) {
