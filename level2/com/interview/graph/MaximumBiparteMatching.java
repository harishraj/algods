...
public class MaximumBiparteMatching {
	...
	public int findMaxMatching(Map<Integer, List<Integer>> jobApplications,
			List<Integer> allJobs) {
		...
		for (Integer candidate : jobApplications.keySet()) {
			...
			maxMatch += matchJobs(candidate, jobApplications, match, jobsSeen) == true ? 1
					: 0;
		}
		return maxMatch;
	}
	...
	private boolean matchJobs(Integer candidate,
			Map<Integer, List<Integer>> jobApplications,
			Map<Integer, Integer> match, Set<Integer> jobsSeen) {
		...
		for (int job : jobApplications.get(candidate)) {
			...
			boolean flag = matchJobs(match.get(job), jobApplications, match,
					jobsSeen);
			if (flag) {
				match.put(job, candidate);
				return true;
