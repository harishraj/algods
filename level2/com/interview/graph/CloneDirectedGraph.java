 ...
 * Test cases Graph is directed/non directed Graph has 0 edges Graph has cycle
 * Graph is linear Graph is dense Graph is sparse
 */
public class CloneDirectedGraph<T> {
	...
	private void updateMap(Map<Vertex<T>, Vertex<T>> cloneMap,
			Vertex<T> cloneVertex, Vertex<T> adjVertex, Edge<T> edge) {
		if (cloneMap.containsKey(adjVertex)) {
			Vertex<T> adjVertexClone = cloneMap.get(adjVertex);
			Edge<T> newEdge = new Edge<T>(cloneVertex, adjVertexClone,
					edge.isDirected(), edge.getWeight());
