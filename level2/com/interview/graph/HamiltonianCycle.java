...
public class HamiltonianCycle<T> {
	...
	public boolean getHamiltonianCycle(Graph<T> graph, List<Vertex<T>> result) {
		...
		return hamiltonianUtil(startVertex, startVertex, result, visited, graph
				.getAllVertex().size());

	}

	private boolean hamiltonianUtil(Vertex<T> startVertex,
			Vertex<T> currentVertex, List<Vertex<T>> result,
			Set<Vertex<T>> visited, int totalVertex) {
		...
		for (Vertex<T> child : currentVertex.getAdjacentVertexes()) {
			...
			if (!visited.contains(child)) {
				boolean isHamil = hamiltonianUtil(startVertex, child, result,
						visited, totalVertex);
				if (isHamil) {
					return true;
				}
