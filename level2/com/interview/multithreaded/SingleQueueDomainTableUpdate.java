...
/**
 * Given a queue which gets millions of messages. Message is of form
 * <Domain,Update>. You have 10000 domain tables. Also you have 50 worker
 * threads. You can only get data from front of the queue. Threads get data from
 * the front and then update the domain table. If work is being done on domain
 * table you cannot apply another update. Update should also be applied
 * sequentially. So an update coming later on should not be applied before an
 * update coming sooner.
 */
			...
			if (p.yourLock) {
				...
				if (queue != null) {
					while (queue.size() > 0) {
						...
						databaseLayer.applyUpdates(data.getDomain(),
								data.getUpdate());
					}
				}
				databaseLayer.applyUpdates(p.data.getDomain(),
						p.data.getUpdate());
...
interface QueueHandle {
	// this is a blocking call. If there is no data in the queue it just waits
