...
/**
 * Convert a roman number to decimal number Test cases: smaller number appearing
 * before bigger number unless it is representing one less. VL is incorrect
 * since XL should be used to represent 90 chars not used in roman
...
public class RomanNumberToDecimal {
	...
	public int convertToDecimal(char[] roman) {
		...
		for (int i = 0; i < roman.length;) {
			if (i < roman.length - 1
					&& literal(roman[i]) < literal(roman[i + 1])) {
