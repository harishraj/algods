...
/**
 * http://stackoverflow.com/questions/4686823/given-2-sorted-arrays-of-integers-
 * find-the-nth-largest-number-in-sublinear-time
 */
public class KthLargestInTwoSortedArray {

	public int kthLargest1(int arr1[], int arr2[], int low1, int high1,
			int low2, int high2, int k) {
	...
	public int kthLargest(int input1[], int input2[], int k) {
		return kthLargest(input1, input2, 0, input1.length - 1, 0,
				input2.length - 1, k);
	}

	private int kthLargest(int input1[], int input2[], int l1, int h1, int l2,
			int h2, int k) {
		if (l1 > h1) {
			return input2[k - 1];
		}
