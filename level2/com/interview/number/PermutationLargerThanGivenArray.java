...
/**
 * e.g if src array is {1,2,5,7,8} and dest is {5,8,1,9,8} permute src in such a
 * way that it is greater than dest but diff is minimal so here we will have
 * {5,8,2,1,7}
 * 
 */
public class PermutationLargerThanGivenArray {
	...
	private boolean findNumber(int src[], int dest[], int result[], int pos,
			boolean used[]) {
		...
		// first try with equal item and see next numbers in array might find
		// larger one.
