...
public class MinimumEditForReversePolishNotation {
	...
	private int minimum(char input[], int pos, int countXs) {
		if (pos == input.length) {
			// we should be always be left with 1 x otherwise notation is
			// incomplete
			if (countXs > 1) {
		...
		// If input is * we can take 2 xs and conver it to one x. If we dont
		// have
		// 2xs then we can either delete this * or convert it to x.
		// Remember adding anything does not make any sense since deleting
		// things
		// can achieve same things unless cost of adding is different from
		// deleting
		if (input[pos] == 'x') {
