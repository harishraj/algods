...
public class RabinKarpSearch {
	...
	public boolean subString(char[] mainString, char[] subString) {
		...
		if (subStringHash == mainStringHash) {
			if (checkEqual(mainString, 0, subString.length - 1, subString, 0,
					subString.length - 1)) {
				return true;
			}
		}
		for (int i = subString.length; i < mainString.length; i++) {
			mainStringHash = recalculateHash(mainString, i - subString.length,
					i, subString.length, mainStringHash);
			if (subStringHash == mainStringHash) {
				if (checkEqual(mainString, i - subString.length + 1, i,
	...
	private int recalculateHash(char[] str, int oldIndex, int newIndex,
			int size, int hash) {
		hash = hash / prime;
		hash -= str[oldIndex];
	...
	private boolean checkEqual(char str1[], int start1, int end1, char str2[],
			int start2, int end2) {
		int i = start1;
		int j = start2;
		while (i <= end1 && j <= end2) {
	...
	public static void main(String args[]) {
		...
		System.out.println(rks.subString("Tushar".toCharArray(),
				"shas".toCharArray()));
	}
}
