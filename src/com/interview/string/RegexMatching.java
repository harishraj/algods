...
public class RegexMatching {
	...
	private boolean match(char[] str, char pattern[], int pos1, int pos2) {
		...
		if (pos2 + 1 >= pattern.length || pattern[pos2 + 1] != '*') {
			if (pattern[pos2] == '.') {
				...
				return str[pos1] == pattern[pos2]
						&& match(str, pattern, pos1 + 1, pos2 + 1);
			}
		}
	...
	public static void main(String args[]) {
		...
		System.out.println(rm.match("Tushar".toCharArray(),
				"Tushar".toCharArray()));
		System.out.println(rm.match("Tusha".toCharArray(),
				"Tushar*a*b*".toCharArray()));
		System.out.println(rm.match("".toCharArray(), "a*b*".toCharArray()));
		System.out.println(rm.match("abbbbccc".toCharArray(),
