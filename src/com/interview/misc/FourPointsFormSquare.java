...
/**
 * Given four points in any order determine if they form a square Test cases
 * Less than or more than 4 points in input Points could be in any quadrants
 * e.it neg and pos allowed
 */
public class FourPointsFormSquare {
	...
	private boolean compare(Cordinate startPoint, Cordinate point1,
			Cordinate point2, int len, int diag) {
		if (2 * len != diag) {
			return false;
		}
	...
	private int distanceSquare(Cordinate c1, Cordinate c2) {
		return (int) (Math.pow(Math.abs(c1.x - c2.x), 2) + Math.pow(
