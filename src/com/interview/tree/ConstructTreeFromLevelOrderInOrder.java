 ...
 * Test cases same length of inorder and level order array all elements should
 * be same in them what to do if repetition happens. This logic only works for
 * non repeated values if inorder and levelorder were represented by string how
 * would you handle multi digits e.g 1234 we don't know if it is 12 34 or 1 2 3
 * 4 or what. Maybe use brackets (12)(3)(4) to differentiate between them.
 */
public class ConstructTreeFromLevelOrderInOrder {
	...
	private int[] extractArray(int inOrder[], int levelOrder[], int low,
			int high) {
	...
	private Node constructTree(int inOrder[], int levelOrder[], int low,
			int high) {
		if (low > high) {
			return null;
		}
