...
public class ArrayMultiplication {
	...
	public int[] multiplyDivideAndConquer(int arr1[], int arr2[], int low1,
		...
		int r2[] = multiplyDivideAndConquer(arr1, arr2, mid1 + 1, high1,
				mid2 + 1, high2);

		int r3[] = multiplyDivideAndConquer(arr1, arr2, low1, mid1, mid2 + 1,
				high2);
		shiftBy = high1 - mid1;
		r3 = shift(r3, shiftBy);

		int r4[] = multiplyDivideAndConquer(arr1, arr2, mid1 + 1, high1, low2,
				mid2);
		shiftBy = high2 - mid2;
		r4 = shift(r4, shiftBy);
	...
	public int[] simpleMultiplication(int arr1[], int arr2[], int low1,
			int high1, int low2, int high2) {
	...
	public int multiplicationImproved(int x, int y, int len) {
		...
		int result3 = multiplicationImproved(x / div + x % div, y / div + y
				% div, len);

		return result1 * (power(2 * len) - power(len)) + result3 * power(len)
				- result2 * (power(len) - 1);
	...
	public static void main(String args[]) {
		...
		int result[] = am.multiplyDivideAndConquer(arr1, arr2, 0,
				arr1.length - 1, 0, arr2.length - 1);
