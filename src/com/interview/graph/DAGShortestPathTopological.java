...
public class DAGShortestPathTopological<T> {
	...
	public Map<Vertex<T>, Integer> shortestPath(Graph<T> graph,
		...
		while (!deque.isEmpty()) {
			...
			for (Edge<T> edge : vertex.getEdges()) {
				if (getDistance(edge.getVertex2(), distance) > getDistance(
						edge.getVertex1(), distance)
						+ edge.getWeight()) {
					distance.put(
							edge.getVertex2(),
							getDistance(edge.getVertex1(), distance)
									+ edge.getWeight());
				}
			}
		}
	...
	public static void main(String args[]) {
		...
		Map<Vertex<Integer>, Integer> distance = shortestPath.shortestPath(
				graph, graph.getAllVertex().iterator().next());
