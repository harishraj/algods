...
public class BellmanFordShortestPath {
	...
	public Map<Vertex<Integer>, Integer> getShortestPath(Graph<Integer> graph,
		...
		for (int i = 0; i < graph.getAllVertex().size() - 1; i++) {
			for (Edge<Integer> edge : graph.getAllEdges()) {
				...
				if (getDistance(u, distance) > getDistance(v, distance)
						+ edge.getWeight()) {
					distance.put(u, getDistance(v, distance) + edge.getWeight());
				}
				if (getDistance(v, distance) > getDistance(u, distance)
						+ edge.getWeight()) {
	...
	public static void main(String args[]) {
		...
		Map<Vertex<Integer>, Integer> distance = shortestPath.getShortestPath(
				graph, startVertex);
