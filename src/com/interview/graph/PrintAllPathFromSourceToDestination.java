...
/**
 * http://www.careercup.com/question?id=5922416572235776 Test case Source or
 * destination vertex does not exist in the graph There is no path between src
 * to dest vertex
 */
public class PrintAllPathFromSourceToDestination {

	public void printPath(Graph<Integer> graph, Vertex<Integer> start,
			Vertex<Integer> destination) {
		Set<Vertex<Integer>> visited = new LinkedHashSet<Vertex<Integer>>();
		printPath(visited, destination, start);
	}

	private void printPath(Set<Vertex<Integer>> visited,
			Vertex<Integer> destination, Vertex<Integer> current) {
